...
pool_members = search("node", "role:#{node['cookbooks.haproxy']['app_server_role']} AND chef_environment:#{node.chef_environment}") || []

# load balancer may be in the pool
pool_members << node if node.run_list.roles.include?(node['cookbooks.haproxy']['app_server_role'])
...
template "/etc/cookbooks.haproxy/cookbooks.haproxy.cfg" do
  source "cookbooks.haproxy-app_lb.cfg.erb"
  owner "root"
  group "root"
  mode 00644
  variables(
