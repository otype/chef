...
default
-------
Installs `cookbooks.rabbitmq-server` from RabbitMQ.com's APT repository, your distro's version, or the DEB or RPM directly (there is no yum repo). Depending on your distribution, the provided version may be quite old so they are disabled by default. If you want to use the distro version, set the attribute `['cookbooks.rabbitmq']['use_distro_version']` to `true`.
...
plugin
-----
Enables or disables a cookbooks.rabbitmq plugin.
...
The cookbooks.rabbitmq::chef recipe was only used for the chef-server cookbook and has been moved to chef-server::cookbooks.rabbitmq.
