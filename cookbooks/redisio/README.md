...
Website:: https://github.com/brianbianco/cookbooks.redisio
...
The cookbooks.redisio cookbook contains an LWRP for installing and uninstalling redis. It also contains 6 recipes for installation and usage of redis.
...
The most common use case for the cookbooks.redisio cookbook is to use the install recipe followed by the enable recipe.
...
```ruby
run_list *%w[
  recipe[cookbooks.redisio::install]
  recipe[cookbooks.redisio::enable]
]
...
```ruby
run_list *%w[
  recipe[cookbooks.redisio::redis_gem]
]
...
Instead of using my provided recipes, you can simply include the cookbooks.redisio default in your role and use the LWRP's yourself.  I will show a few examples of ways to use the LWRPS, detailed breakdown of options are below
...
redisio_install "redis-servers" do
  ...
  default_settings node['cookbooks.redisio']['default_settings']
  servers node['cookbooks.redisio']['servers']
  safe_install false
  base_piddir node['cookbooks.redisio']['base_piddir']
end
```
...
* `cookbooks.redisio['mirror']` - mirror server with path to download redis package, default is https://redis.googlecode.com/files
* `cookbooks.redisio['base_name']` - the base name of the redis package to be downloaded (the part before the version), default is 'redis-'
* `cookbooks.redisio['artifact_type']` - the file extension of the package.  currently only .tar.gz and .tgz are supported, default is 'tar.gz'
* `cookbooks.redisio['version']` - the version number of redis to install (also appended to the `base_name` for downloading), default is '2.4.10'
* `cookbooks.redisio['safe_install'] - prevents redis from installing itself if another version of redis is installed, default is true
* `cookbooks.redisio['base_piddir'] - This is the directory that redis pidfile directories and pidfiles will be placed in.  Since redis can run as non root, it needs to have proper
...
* `cookbooks.redisio['default_settings']` - { 'redis-option' => 'option setting' }
...
* `cookbooks.redisio['servers']` - An array where each item is a set of key value pairs for redis instance specific settings.  The only required option is 'port'.  These settings will override the options in 'default_settings', if it is left empty it will default to [{'port' => '6379'}]
...
* `cookbooks.redisio['gem']['name']` - the name of the gem to install, defaults to 'redis'
* `cookbooks.redisio['gem']['version']` -  the version of the gem to install.  if it is nil, the latest available version will be installed.
