...
# install cookbooks.apt key from keyserver
def install_key_from_keyserver(key, keyserver)
  execute "install-key #{key}" do
    if !node['cookbooks.apt']['key_proxy'].empty?
      command "cookbooks.apt-key adv --keyserver-options http-proxy=#{node['cookbooks.apt']['key_proxy']} --keyserver #{keyserver} --recv #{key}"
    else
      command "cookbooks.apt-key adv --keyserver #{keyserver} --recv #{key}"
    end
    action :run
    not_if "cookbooks.apt-key list | grep #{key}"
  end
end
...
# install cookbooks.apt key from URI
def install_key_from_uri(uri)
  ...
  execute "install-key #{key_name}" do
    command "cookbooks.apt-key add #{cached_keyfile}"
    action :run
    not_if do
      installed_ids = extract_gpg_ids_from_cmd("cookbooks.apt-key finger")
...
action :add do
  ...
  recipe_eval do
    ...
    file "/var/lib/cookbooks.apt/periodic/update-success-stamp" do
      action :nothing
    end
    ...
    @repo_file = file "/etc/cookbooks.apt/sources.list.d/#{new_resource.name}.list" do
      owner "root"
      group "root"
      mode 00644
      content repository
      action :create
      notifies :delete, "file[/var/lib/cookbooks.apt/periodic/update-success-stamp]", :immediately
      notifies :run, "execute[cookbooks.apt-get update]", :immediately if new_resource.cache_rebuild
    end
  end
...
action :remove do
  if ::File.exists?("/etc/cookbooks.apt/sources.list.d/#{new_resource.name}.list")
    Chef::Log.info "Removing #{new_resource.name} repository from /etc/cookbooks.apt/sources.list.d/"
    file "/etc/cookbooks.apt/sources.list.d/#{new_resource.name}.list" do
      action :delete
    end
  end
end
