...
This cookbook includes recipes to execute cookbooks.apt-get update to ensure the
local APT package cache is up to date. There are recipes for managing
the cookbooks.apt-cacher-ng caching proxy and proxy clients. It also includes a
LWRP for managing APT repositories in /etc/cookbooks.apt/sources.list.d as well as
an LWRP for pinning packages via /etc/cookbooks.apt/preferences.d.
...
the timestamp file used to only run `cookbooks.apt-get update` if the cache is
...
Installs the `cookbooks.apt-cacher-ng` package and service so the system can
...
cacher-client
-------------
Configures the node to use the `cookbooks.apt-cacher-ng` server as a client. If you
want to restrict your node to using the `cookbooks.apt-cacher-ng` server in your
Environment, set `['cookbooks.apt']['cacher-client']['restrict_environment']` to `true`.
To use a cacher server (or standard proxy server) not available via search
set the atttribute `['cookbooks.apt']['cacher-ipaddress']` and for a custom port
set `['cookbooks.apt']['cacher_port']`
...
Adding a new repository will notify running the `execute[cookbooks.apt-get-update]`
...
This LWRP provides an easy way to pin packages in /etc/cookbooks.apt/preferences.d.
Although cookbooks.apt-pinning is quite helpful from time to time please note that Debian
does not encourage its use without thorough consideration.

Further information regarding cookbooks.apt-pinning is available via
http://wiki.debian.org/AptPreferences.

# Actions

- :add: creates a preferences file under /etc/cookbooks.apt/preferences.d
...
Put `recipe[cookbooks.apt]` first in the run list. If you have other recipes
that you want to use to configure how cookbooks.apt behaves, like new sources,
notify the execute resource to run, e.g.:

    template "/etc/cookbooks.apt/sources.list.d/my_apt_sources.list" do
...
Put `recipe[cookbooks.apt::cacher-ng]` in the run_list for a server to provide
APT caching and add `recipe[cookbooks.apt::cacher-client]` on the rest of the
Debian-based nodes to take advantage of the caching server.

If you want to cleanup unused packages, there is also the `cookbooks.apt-get autoclean`
and `cookbooks.apt-get autoremove` resources provided for automated cleanup.
