...
%w( wget zlib1g-dev libssl-dev libffi-dev libxml2-dev libncurses5-dev libreadline5-dev libtool libyaml-dev).each do |pkg|
  package pkg do
    action :install
  end
end
...
%w( ohai chef ruby-shadow rails).each do |g|
  gem_package g do
    action :install
